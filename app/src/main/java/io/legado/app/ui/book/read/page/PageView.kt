...
//        setPadding(paddingLeft, context.statusBarHeight, paddingRight, paddingBottom)
    ...
    private fun initAccessibility() {
        // 设置页面视图的基本无障碍属性
        this.contentDescription = context.getString(R.string.reading_page)
        this.importantForAccessibility = View.IMPORTANT_FOR_ACCESSIBILITY_YES
        
        // 为内容文本视图添加无障碍支持
        binding.contentTextView.apply {
            contentDescription = context.getString(R.string.book_content)
        ...
        listOf(
            ...
            textView.contentDescription = context.getString(R.string.page_info)
     ...
     */
    fun updateAccessibilityInfo(pageNumber: Int, totalPages: Int) {
        // 使用 AccessibilityEnhancer 为阅读页面提供专门的无障碍支持
        io.legado.app.accessibility.AccessibilityEnhancer.enhanceReadingPageView(
            this,
            context,
            pageNumber,
            totalPages
        ...
        // 更新内容文本视图的描述
        binding.contentTextView.contentDescription = context.getString(
            R.string.a11y_reading_page_content,
            pageNumber,
            totalPages
        )
    }
    ...
    /**
     * 确保触摸目标大小符合无障碍标准（最小48dp）
     */
    private fun ensureTouchTargetSize() {
        ...
        binding.contentTextView.apply {
            ...
            if (currentWidth < minSizePx || currentHeight < minSizePx) {
                val paddingHorizontal = if (currentWidth < minSizePx) {
                    (minSizePx - currentWidth) / 2
                } else {
                    0
                }
                
                val paddingVertical = if (currentHeight < minSizePx) {
                    (minSizePx - currentHeight) / 2
                } else {
                    0
                }
